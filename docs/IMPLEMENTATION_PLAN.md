...
**Slice 2 — not started, next session:**

- [ ] Assumptions/trade-offs panel
- [ ] Finalization flow: a proposal-hash mechanism (nothing computes one yet — `PROJECT_BRIEF.md`'s `approval_records.proposal_hash` / the state machine's `proposalHashMatches` precondition for `user_confirmed` both assume one exists), the confirmation UI, and "No booking made" messaging
- [ ] Trip history list (`/app/trips`) polish — the route exists (Phase 0) but is a bare list, not yet linked to what a trip's status/draft actually looks like
- [ ] Browser-verify the chat UI + live itinerary panel once a dev server is free and the Realtime migration is applied
...
- [ ] **The chat UI and live itinerary panel (Phase 7) haven't been browser-verified.** Another session already held this project's one allowed `next dev` instance for this entire session (Next.js enforces a single dev server per project directory, not just per port). The Realtime migration is now applied (see "Known bugs — resolved") so nothing else should block a real click-through check once a dev server is free. `.claude/launch.json` gained `"autoPort": true` this session (harmless, but didn't address this specific blocker, which is the per-directory lock, not the port).
