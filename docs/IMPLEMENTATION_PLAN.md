...
## STEPWISE CHAIN REDESIGN (decided 2026-09-17; slices 1-2 built 2026-09-17)

**Slices 1-2 (chain/cascade data model + flight step; hotel step + the flight→hotel cascade rule) are done.** Slices 3-4 remain — see "Proposed slicing" below. Once slice 3 retires the old one-shot path, go back and fix whatever it broke in already-shipped code (search-orchestrator.ts, itinerary-orchestrator.ts, intake-orchestrator.ts, the chat UI, and their tests all still assume the one-shot model), then resume Phase 6/7/beyond as originally sequenced.
...
**Proposed slicing (order locked through slice 2 — revisit before committing to slice 3's scope, e.g. exactly how retiring the old path interacts with `intake-orchestrator.ts`'s decision-revision detection):**
...
- [ ] **The stepwise chain (`flight-step.ts`/`hotel-step.ts`) has no chat-level way to *trigger* a step revision yet — only the underlying mechanics exist.** `confirmFlightStep`/`confirmHotelStep` can be called again directly (they retire-then-append, and the flight→hotel cascade rule is real and unit/live-tested as of slice 2), but nothing yet detects "the user wants to change a flight/hotel they already confirmed" from a chat message — that needs `intake-orchestrator.ts` to become step-aware, still a later-slice item (see "What this replaces" above). The activities step doesn't exist yet either, so the hotel→activities non-cascade rule (rule 3) has no real decision to leave untouched — that's slice 3's job.
