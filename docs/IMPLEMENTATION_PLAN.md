...
#### Phase 6, continued — wiring search, curation, and combination (not started)

Phase 5 built the Curator/Explanation agents, retrieval, and (already, from Phase 2) the budget/combination/feasibility engines — **none of them are wired into any orchestrator yet.** Confirmed by grep immediately before writing this, so this isn't a guess: no `src/workflow/`, `src/agents/`, or `app/` code calls `calculateBudget`, `assembleCandidateCombinations`, `validateItineraryFeasibility`, `validateInventoryReferences`, `findFlights`, or `findHotels`; nothing outside their own test files calls `runCuratorAgent`/`runExplanationAgent`. The state machine already models the transitions this needs to drive (`requirements_ready` → `searching_inventory` → `validating_candidates` → `assembling_options` → `validating_itinerary` → `presenting_draft`, `src/workflow/state-machine.ts`), but nothing calls `advanceTrip` with any of those events yet — only `start_intake`/`clarification_needed`/`clarification_resolved`/`requirements_complete` are driven today, by `intake-orchestrator.ts`.

**Start here (next session):** this gap was identified while closing out documentation, not discussed or scoped in detail — treat the items below as open questions to resolve before writing code, not decisions already made:

1. **`search_flights`/`search_hotels`: deterministic pre-fetch, or live model-driven tool calls?** Phase 5 decided the Curator agent doesn't call `retrieve_destinations`/`retrieve_activities` as live tool calls mid-turn — the orchestrator fetches candidates first, deterministically, and hands results in. The same question applies here, and leaning toward the same answer for consistency (search parameters — origin/destination/dates/party size — come straight from validated `trip_requirements`, no model judgment needed to construct the search itself), but `PROJECT_BRIEF.md` §12 does list `search_flights`/`search_hotels` as model-facing tools, so this is worth explicitly confirming rather than silently assuming Phase 5's precedent extends here too.
2. **New orchestrator module, or extend `intake-orchestrator.ts`?** The Intake orchestrator is scoped to the `collecting_requirements`/`awaiting_clarification` loop specifically. A separate module (e.g. `search-orchestrator.ts`) driving `requirements_ready` onward is the more consistent split (mirrors `runIntakeAgent`/`runCuratorAgent` already being separate agent modules), but not yet decided.
3. **No `trip_decisions` repository exists yet.** Needed to persist what combination/feasibility actually selects. Does it need the same `append`/`listActive`/`retireActiveXForField` shape `trip_requirements`/`trip_preferences` already use (Phase 6), or does a decision warrant different persistence semantics (e.g. tied to a specific proposal/version rather than "current value per field," given §7.6's approval-invalidation-on-revision model)? Not decided.
4. **Scope for one session.** The full chain (search → curate → combine budget/constraints → validate feasibility → persist decisions → present draft) is a lot to wire at once — comparable in size to Phase 6's Intake wiring, plus a review pass. Worth explicitly deciding whether to slice it further (e.g. search+curate first, combination+feasibility as a follow-up) rather than assuming it's one block of work, the same way Phase 4/6/5 each stayed intentionally scoped.

- [ ] Wire `search_flights`/`search_hotels` (resolve question 1 above first)
- [ ] Wire the Curator agent into a real orchestrator step — candidates in (destinations/activities from Phase 5, flights/hotels from the item above), curation out, persisted
- [ ] Wire `assembleCandidateCombinations`/`calculateBudget` (Phase 2, still completely unused outside their own tests)
- [ ] Wire `validateItineraryFeasibility`/`validateInventoryReferences` as guardrails before `presenting_draft` (Phase 2, same — unused outside their own tests)
- [ ] `trip_decisions` repository (resolve question 3 above first)
- [ ] Drive the remaining state-machine transitions: `begin_search` → `search_completed` → `candidates_valid` → `combinations_assembled` → `itinerary_valid`/`itinerary_invalid`
