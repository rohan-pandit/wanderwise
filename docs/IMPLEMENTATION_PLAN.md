...
- [ ] `trip_decisions` repository (shape resolved above — append/retire, `proposed`/`confirmed`/`superseded`)
- [ ] Wire `assembleCandidateCombinations`/`calculateBudget` (Phase 2, still completely unused outside their own tests) over the flights/hotels/activities `runSearchAndCuration` now returns
...
- [ ] Persist the assembled selection to `trip_decisions`
- [ ] Drive the remaining state-machine transitions: `combinations_assembled` → `itinerary_valid`/`itinerary_invalid`
