...
**Proposed slicing (order not fully locked in beyond slice 1 — revisit before committing to slice 2):**
...
- [ ] **The new flight step (`src/workflow/flight-step.ts`, stepwise chain redesign slice 1, 2026-09-17) has no way to revise an *already-confirmed* flight decision.** `confirmFlightStep` can be called again with a different pair (it retires-then-appends, same as every other decision write), but nothing yet detects "the user wants to change a flight they already confirmed" from a chat message, and `src/domain/chain.ts`'s cascade functions (`invalidatedStepsForFlightChange`, etc.) have no live caller — there's nothing downstream to invalidate yet since hotel/activities steps don't exist. Deliberately deferred: building the cascade-aware revision path only makes sense once slice 2/3 gives it something real to cascade to.
