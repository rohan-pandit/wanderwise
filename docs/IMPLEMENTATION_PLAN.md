...
## STEPWISE CHAIN REDESIGN (decided 2026-09-17; slices 1-3 built 2026-09-17)
...
**4. What this replaces, concretely:**
   ...
   - [ ] `intake-orchestrator.ts`'s decision-revision detection (`decisionRevisionRequested` signal, `RevisableDecisionField`) — still not step-aware (still a flat field-name signal, unchanged from the old pipeline) — slice 4.
   - [ ] The chat UI (`chat-panel.tsx`, `itinerary-panel.tsx`) — still shows the whole trip appearing/updating live with no per-step confirm/change affordances; `sendMessage`'s auto-chain (`app/app/actions.ts`) now calls the new `chain-orchestrator.ts` functions but still auto-confirms every step's top pick rather than waiting for the user — slice 4 replaces this interim glue entirely.
   - [ ] State machine impact, decided direction (not yet implemented): **don't** explode `src/workflow/state-machine.ts` into per-step states (`awaiting_flight_confirmation` etc.) — keep the trip-level states coarse. As built (slices 1-3), the trip-level `WorkflowState` never advances past `requirements_ready` at all — every step lives entirely in `trip_decisions.status` (`"proposed"` → `"confirmed"` → `"superseded"`, still only ever `"confirmed"` in practice, since no step currently uses the `"proposed"` intermediate — see slice 1's design call) plus `getCurrentChainStep`. Whether `presenting_draft`/`awaiting_confirmation`/`finalized` ever get reached at all under the new model — and what "finalizing" a trip even means now — is still an open question for slice 4.

**Proposed slicing — only slice 4 remains:**
...
4. Chat UI rework for the step-by-step interaction pattern (one step shown at a time, confirm/change per step) — this also means deleting `chain-orchestrator.ts`'s interim auto-confirm glue and making `intake-orchestrator.ts`'s decision-revision detection step-aware (currently a flat field-name signal, per "What this replaces" item 4 below, unchanged).
...
- [ ] **A chat-requested step revision works end to end now (slice 3, live-verified), but only via interim, non-criteria-aware glue.** `intake-orchestrator.ts` still isn't step-aware — it just detects "the user wants to change flight/hotel" as a flat field-name signal, unchanged from the old pipeline — and `chain-orchestrator.ts`'s `runStepwiseRevision` (explicitly throwaway) responds by excluding the current pick and auto-confirming whatever's next-cheapest, the same shape of limitation item below (`decision-revision loop doesn't honor the specific criterion`) describes, just on new code. Real step-aware revision detection, plus honoring the user's actual stated criterion ("cheaper," "closer to the center," etc.), is slice 4's job.
- [ ] **The flight→hotel cascade's "dates changed" branch (`confirmFlightStep` retiring a confirmed hotel decision, `flight-step.ts`) is only verified live in one direction.** The seed data has exactly one Lisbon→New York return-leg flight (confirmed by a direct query during slice 2's live spot-check), so there's no second real return date to swap in and exercise "confirming a flight with different derived dates actually invalidates the hotel" against live data — only the "same dates → untouched" direction was live-verified. The "dates differ → invalidated" branch is still covered by 3 dedicated unit tests with synthetic data. Revisit if/when more return-leg seed data gets added (same root cause as the `0006_return_flights.sql` gap Phase 6 continued slice 2 found and fixed for the *old* pipeline — this is the analogous gap for the new one, on a different route).
- [ ] **The decision-revision loop doesn't honor the specific criterion in a revision request, only the field being changed.** Found live under the old pipeline (Phase 7 slice 1, 2026-09-16): asking to "switch hotels" (vague) correctly triggers a clarifying follow-up, but once a reason is given (e.g. "pick a cheaper hotel"), the revision only excludes the current pick and takes whatever's next by ranking — it never looks at the free-text criterion (`RevisionProposal.value`) at all. **Confirmed the same limitation still holds under the new stepwise chain** (slice 3, `chain-orchestrator.ts`'s `runStepwiseRevision`, live-verified 2026-09-17): excludes the current hotel and auto-confirms the next-cheapest passing candidate, coincidentally matching "cheaper" this time only because cheapest-first is the step's own default ranking — a request like "a nicer hotel" or "closer to the center" would still be ignored the same way. Not fixed (deliberately, both times): pattern-matching an unstructured free-text value is fragile and against this project's "don't use fuzzy heuristics for domain decisions" ethos. The real fix is the design's already-decided directional/absolute revision split (`docs/IMPLEMENTATION_PLAN.md`'s "STEPWISE CHAIN REDESIGN" §3) — converting "cheaper" into a concrete `maxHotelPriceUsd` requirement revision — which needs `intake-orchestrator.ts` to become step-aware first (slice 4).
