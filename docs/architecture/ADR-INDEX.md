...
- [ ] Security and prompt-injection handling — adopted in principle (brief §6.6). Partially implemented: input-size limits for the Intake agent (Phase 6, `src/workflow/intake-orchestrator.ts`'s Layer 1 guardrail) and retrieved-content delimiting/untrusted-labeling for the Curator/Explanation agents (Phase 5, §10.4 — `<candidate_data>`/`<explanation_data>` blocks, explicit "not instructions" framing in both system prompts). Adversarial testing against actual injection attempts now exists for both surfaces (`evals/cases/scenarios.ts`'s `out_of_scope_request` for chat-message injection; `evals/cases/adversarial.ts`'s `prompt_injection_in_retrieved_text` for retrieved-content injection, added 2026-09-17 — confirmed live that the Curator correctly identified and excluded an injected activity). Still open: a deterministic scope classifier (non-travel request detection). Not yet an ADR.
...
- [x] Retry and recovery policy — implemented for the transition layer (`advanceTrip`'s per-write correlation-ID guards, Phase 3) and reused for multi-hop transitions within one orchestrator turn (Phase 6, `src/workflow/intake-orchestrator.ts`'s event-name-keyed chaining — an index-keyed version of this had a real retry-breaking bug, found and fixed during Phase 6's review). A retried orchestrator turn's non-transition writes are guarded where it's unambiguously correct to do so — `messages.correlation_id` (resolved 2026-09-17) — but deliberately *not* `agent_runs`/`tool_calls`/`guardrail_events`, since a genuine retry's second real LLM call is a real, separately-costed event; deduplicating those would make the cost/observability dashboards under-report real spend. A user-chosen scope narrowing, not an oversight.
- [x] Cancellation behavior — adopted from brief §8.2/§8.4 (Phase 2's wildcard-`from` `cancel` rule); a real caller built 2026-09-17: `cancelTrip` (`app/app/actions.ts`), a "Cancel trip" UI action with a confirm-before-acting banner, and `requireOwnedTrip` refusing every other mutating action on a cancelled trip by default. Browser-verified live.
...
- [x] Evaluation dataset and scenario replay — decided, Phase 8 slice 1, 2026-09-17: `evals/lib/scenario-harness.ts` + `evals/cases/scenarios.ts` drive the real stepwise chain against a real Supabase trip + real Anthropic/Voyage calls, persisted to `eval_runs`/`eval_results`. **Full 16/16 of brief §19's scenarios reached 2026-09-17** — the remaining 10 mostly just needed new cases against the existing harness (not new infrastructure, as first assumed); two needed one new harness capability each (`signInAs` for RLS-scoped sessions; a temporary seeded fixture for injected text); one (`flexible_destination`, #6) doesn't apply to the current required-field architecture and is documented as such rather than silently dropped. `npm run eval:scenarios`: 20/20 passing live.
- [x] Adversarial testing approach — decided, Phase 8 slice 3, 2026-09-17: `evals/cases/adversarial.ts`, run by the same harness. 6 of §9.6's 8 categories now covered as of 2026-09-17 (fabricated inventory IDs, budget-exceeding wording, contradictory user messages, cross-session references, prompt injection in retrieved inventory text, full RLS cross-user isolation); "attempts to trigger booking" already covered by `scenarios.ts`'s `out_of_scope_request`. Malformed/malicious inventory *records* (as opposed to injection-laden but well-formed text) remain untested — tracked, not yet built.
